...
inherit check-reqs cmake-utils flag-o-matic gnome3 pax-utils python-any-r1 ruby-single toolchain-funcs virtualx
...
GITHUB_REPO="webkit"
GITHUB_USER="bngo11"
GITHUB_TAG="e1a22cace7ba1fdcef244e5c54a612ae397cda49"
...
IUSE="debug +introspection gles2 systemd wayland +X"

# "strip" is needed in RESTRICT for debugging...
RESTRICT="test userpriv"

# !<=net-libs/webkit-gtk-2.28.0:
...
RDEPEND="
	...
	wayland? (	>=gui-libs/libwpe-1.3.0:1.0
...
# 	fi
	...
	local mycmakeargs=(
		...
		-DENABLE_GLES2=$(usex gles2 ON OFF)
		-DENABLE_INTROSPECTION=$(usex introspection ON OFF)
		-DENABLE_X11_TARGET=$(usex X ON OFF)
		-DENABLE_WEB_RTC=OFF
		-DENABLE_MEDIA_STREAM=OFF
		-DENABLE_WAYLAND_TARGET=$(usex wayland ON OFF)
		-DUSE_WPE_RENDERER=${wpe_enabled}
		-DUSE_SYSTEMD=${systemd_enabled}
	)
	${S}/Tools/Scripts/set-webkit-configuration --force-optimization-level=none || die
	if use debug; then
		${S}/Tools/Scripts/build-webkit --gtk --debug --prefix=/usr --makeargs="${MAKEOPTS}" --cmakeargs="${mycmakeargs[*]}" || die
	else
		${S}/Tools/Scripts/build-webkit --gtk --release --prefix=/usr --makeargs="${MAKEOPTS}" --cmakeargs="${mycmakeargs[*]}" || die
	fi
}
