...
inherit multiprocessing python-any-r1 versionator toolchain-funcs multilib multilib-minimal
...
SRC_URI="https://static.rust-lang.org/dist/${SRC} -> rustc-${PV}-src.tar.gz
		...
		https://static.rust-lang.org/dist/${RUST_STAGE0_x86}.tar.gz
"
...
src_prepare() {
	...
	multilib_src_prepare() {
		local rust_stage0_root="${BUILD_DIR}"/rust-stage0

		local rust_stage0_name="RUST_STAGE0_${ABI}"
...
multilib_src_configure() {
	...
	local rust_target_name="CHOST_${ABI}"
	...
	if ! multilib_is_native_abi; then
		sed -i -e 's/gcc -m32/gcc/g' config.toml
	fi
}

multilib_src_compile() {
	"${S}"/x.py build --verbose --config=config.toml "${MAKEOPTS}" || die
}

multilib_src_install() {
	env DESTDIR="${D}" "${S}"/x.py install || die
