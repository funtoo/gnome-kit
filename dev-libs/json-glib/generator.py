...
async def generate(hub, **pkginfo):
	...
	stable = pkginfo.get('stable')
	if stable:
		filtered = [v for v in releases if not v.minor & 1]
	else:
		filtered = releases
	...
	if target:
		...
		version = max([v for v in filtered if v.major == target.major and v.minor == target.minor])
	else:
		version = max(filtered)
